  ...
  const core = window.OtterWebuiCore || {};
  const chromeUi = window.OtterWebuiChrome || {};
  let requestId = 0;
  ...
  function safeParseJson(text) {
    if (core.safeParseJson) return core.safeParseJson(text);
  ...
  function nextFrame() {
    if (core.nextFrame) return core.nextFrame();
  ...
  function safeFileStem(value, fallback = 'otter-graph') {
    if (core.safeFileStem) return core.safeFileStem(value, fallback);
  ...
  function downloadJson(filename, data) {
    if (core.downloadJson) {
      core.downloadJson(filename, data);
      return;
    }
  ...
  function closeMenus() {
    if (chromeUi.closeAllMenus) {
      chromeUi.closeAllMenus();
      return;
    }
  ...
  function bindOptionalClick(id, handler) {
    if (core.bindOptionalClick) {
      core.bindOptionalClick(id, handler);
      return;
    }
  ...
  function setOptionalMenuText(id, text) {
    if (core.setOptionalMenuText) {
      core.setOptionalMenuText(id, text);
      return;
    }
    const el = document.getElementById(id);
    if (!el) return;
  ...
  function searchNodes(options = {}) {
    const input = options.input || els.canvasSearch || els.nodeSearch;
  ...
  function installDragPan() {
    ...
    els.graphShell.addEventListener('wheel', (event) => {
      event.preventDefault();
      const factor = chromeUi.wheelZoomFactor ? chromeUi.wheelZoomFactor(event) : Math.exp(-event.deltaY * 0.0012);
  ...
  function installHoverDropdowns() {
    if (chromeUi.installHoverDropdowns) {
      chromeUi.installHoverDropdowns();
      return;
    }
  ...
  function graphViewportInsets() {
    ...
    if (narrow) {
      return { left: 76, top: 28, right: 24, bottom: 76 };
    }
    if (compact) {
      return { left: 76, top: 30, right: 32, bottom: 76 };
    }
    return { left: 86, top: 32, right: 36, bottom: 76 };
  }
